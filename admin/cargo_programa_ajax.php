	...
	*/
	...
	function Verifica_Campos_Programa_AJAX ($post) {
		...
		if ($_GET['ac'] == "editar") {
			...
			$form->chk_empty($post['idcargo'],1,"o cargo");
  ...
  function Limpa_Programa_AJAX ($post){
	...
	*/
    ...
    return $objResponse->getXML();
  	...
  	/*
	Fun??o: Seleciona_Programa_AJAX
 	Seleciona os programas do cargo e colocam eles dinamicamente na tabela html
	*/
    ...
    $objResponse->loadXML(Limpa_Programa_AJAX($post));
   	...
   	$codigoCargo = $post['idcargo'];

			$list_sql = "	SELECT
											CPROG.*   , CRG.nome_cargo , PROG.* , CRG.idcargo as cargo, SMDL.* , MDL.*
										FROM
           						{$conf['db_name']}cargo_programa CPROG
           							INNER JOIN {$conf['db_name']}cargo CRG ON CPROG.idcargo=CRG.idcargo
           							INNER JOIN {$conf['db_name']}programa PROG ON CPROG.idprograma = PROG.idprograma
	...
	*/
		...
		function Verifica_Pai($idpai,$tipopai,$idfilho,$qtd,$permissao,$post="")
	{
				// vari?veis globais
		global $form;
		global $conf;
		...
		//$objResponse->addAlert("teste");
		
		//$objResponse->addAlert(var_dump($tipopai).var_dump($idpai).var_dump($idfilho).var_dump($qtd).var_dump($permissao).var_dump($post));
		
		// se nao tiver nada no post, da um submit no form para pegar ele
		if ($post == "") {
			$objResponse->addScript("xajax_Verifica_Pai($idpai,$tipopai,$idfilho,$qtd,$permissao,xajax.getFormValues('for'));");
			return $objResponse->getXML();
		}
		else {
			
					//$objResponse->addAlert(var_dump($pai).var_dump($idfilho).var_dump($qtd).var_dump($permissao).var_dump($post));
					
					if($permissao == 0) $permissao = "adicionar"; else if($permissao == 1) $permissao = "editar"; else if($permissao==2) $permissao = "excluir"; else $permissao = "listar";
 					if($tipopai == 0) $tipopai = "modulo" ; else $tipopai = "submodulo";
					
 					$aux = "permissao_".$permissao."_".$tipopai."_".$idpai;
					
 					$programa = "permissao_".$permissao."_programa_".$idfilho;

 									
 					
					if($post["$programa"] == 0)
					{
							$objResponse->addAssign("$aux","checked",false);
					}
		}
