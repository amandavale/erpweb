<?php
  ...
  require_once("../entidades/orcamento.php"); 
	...
	require_once("tef_ajax.php");


  // configurações anotionais
  $conf['area'] = "TEF - Operações"; // área
  ...
  // cria o objeto xajax
	...
	$xajax->registerFunction("Verifica_Campos_Comando_TEF_AJAX");
  ...
  if($flags['action'] == "logout") {
    ...
    if($auth->check_user()) {
      ...
      if(!$auth->check_priv($conf['priv'])) {
        ...
        if($andamento){ $err[] = "Existe uma ECF em aberto no sistema, é preciso efetuar o cancelamento desta antes de prosseguir com as demais operações relacionadas a ECF. <br /><a href=\"".$conf['addr']."/admin/orcamento.php?ac=editarNF&tipo=ECF&semestoque=1&idorcamento=".$andamento[0]['idorcamento']."\">Clique aqui</a> para ir &agrave; p&aacute;gina de cancelamento. "; $flags['action'] = "";}
