...
//inicializa classe
$conta_filial = new conta_funcionario();
$banco = new banco();
...
function Insere_Conta_Bancaria_AJAX ($post) {
	...
	$form->chk_empty($post['agencia_funcionario'], 1, 'Agência');
	$form->chk_empty($post['conta_funcionario'], 1, 'Conta');
	...
	// se nao houveram erros, insere na tabela html
		...
		// conta principal ?
		if ($post['principal_funcionario'] == "1") $principal = "Sim";
		else $principal = "Não";
		...
		$tabela = utf8_encode("
						<table width='100%' cellpadding='5' id='$nome_tabela'>
							<tr>
								<td class='tb_bord_baixo' align='left' width='35%'>
									...
									<input type='hidden' name='agencia_funcionario_$total_contas_bancarias' id='agencia_funcionario_$total_contas_bancarias' value='{$post['agencia_funcionario']}' />
									<input type='hidden' name='agencia_dig_funcionario_$total_contas_bancarias' id='agencia_dig_funcionario_$total_contas_bancarias' value='{$post['agencia_dig_funcionario']}' />
									<input type='hidden' name='conta_funcionario_$total_contas_bancarias' id='conta_funcionario_$total_contas_bancarias' value='{$post['conta_funcionario']}' />
									<input type='hidden' name='conta_dig_funcionario_$total_contas_bancarias' id='conta_dig_funcionario_$total_contas_bancarias' value='{$post['conta_dig_funcionario']}' />
									<input type='hidden' name='principal_funcionario_$total_contas_bancarias' id='principal_funcionario_$total_contas_bancarias' value='{$post['principal_funcionario']}' />

									{$info_banco['nome_banco']} - {$info_banco['codigo_banco']}
								</td>
								<td class='tb_bord_baixo' align='center' width='15%'>{$post['agencia_funcionario']}-{$post['agencia_dig_funcionario']}</td>
								<td class='tb_bord_baixo' align='center' width='15%'>{$post['conta_funcionario']}-{$post['conta_dig_funcionario']}</td>
			...
			$objResponse->addClear("agencia_funcionario", "value");
			$objResponse->addClear("agencia_dig_funcionario", "value");
			$objResponse->addClear("conta_funcionario", "value");
			$objResponse->addClear("conta_dig_funcionario", "value");
...
function Deleta_Conta_Bancaria_AJAX ($codigoTabela) {
	...
	$objResponse->addConfirmCommands(1, utf8_encode("Deseja excluir esta conta bancária do funcionario ?"));
 ...
 */
function Seleciona_Conta_Bancaria_AJAX ($codigoFuncionario) {
	...
	$list_sql = "	SELECT
									...
									WHERE
										CFOR.idfuncionario = $codigoFuncionario
									ORDER BY
										CFOR.principal_funcionario DESC
								";
