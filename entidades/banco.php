  	...
  	*/
		...
		*/
		...
		function Busca_Generica ( $pg, $rppg, $busca = "", $ordem = "", $url = ""){
			...
			if ($ordem == "") $ordem = " ORDER BY BNC.nome_banco ASC";
			...
			$list_sql = "	SELECT
											BNC.*
										FROM
           						{$conf['db_name']}banco BNC
										WHERE

           					 	UPPER(BNC.nome_banco) LIKE UPPER('%{$busca}%')
		...
		/**
		  método: Filtra_Banco_AJAX
		  propósito: Filtra_Banco_AJAX
		*/

		function Filtra_Banco_AJAX ( $filtro, $campoID ) {
			...
			$list_sql = "	SELECT
										...
										FROM
											{$conf['db_name']}banco BNC
										WHERE
											UPPER(BNC.nome_banco) LIKE UPPER('%{$filtro}%')
											  OR
											UPPER(BNC.codigo_banco) LIKE UPPER('%{$filtro}%')
										ORDER BY
											BNC.nome_banco ASC ";
				...
				if ($db->num_rows($list_q) > 0) {
					...
					<table width="100%" cellpadding="5" cellspacing="2">
						<tr onselect="" class="cabecalho">
							<td width="85%" class="cabecalho_negrito"><?php echo ('Banco'); ?></td>
							<td class="cabecalho_negrito" align="center"><?php echo ('Codigo'); ?></td>
						</tr>
					<?php
					...
					while($list = $db->fetch_array($list_q)){
						...
						$list['nome_banco_negrito'] = preg_replace("'$filtro'i","<span class='substring_negrito'>\\0</span>", $list['nome_banco']);
						$list['codigo_banco_negrito'] = preg_replace("'$filtro'i","<span class='substring_negrito'>\\0</span>", $list['codigo_banco']);

						?>
						<tr onselect="
							this.text.value = '<?php echo ($list['nome_banco']); ?>';
							$('<?php echo $campoNomeTemp; ?>').value = '<?php echo ($list['nome_banco']); ?>';
							$('<?php echo $campoID; ?>').value = '<?php echo ($list['idbanco']); ?>';
							$('<?php echo $campoFlag; ?>').className = 'selecionou'
						">
							<td class="tb_bord_baixo"><?php echo ($list['nome_banco_negrito']); ?></td>
							<td class="tb_bord_baixo" align="center"><?php echo ($list['codigo_banco_negrito']); ?></td>
						</tr>
						<?php
		...
		*/
		function update($idbanco, $info){
   		...
   		//varre o formulário e monta a consulta;
			...
			$update_sql .= " WHERE  idbanco = $idbanco ";
		...
		*/
		...
		function make_list_select( $filtro = "", $ordem = " ORDER BY nome_banco ASC") {
