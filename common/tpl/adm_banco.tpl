...
{include file="div_erro.tpl"}
  ...
  {if $flags.action == "listar"}
		...
		{if count($list)}
			...
			<table width="95%" align="center">
				...
				<tr>
					...
					<th align='center'>Nome do banco</th>
					<th align='center'>Nº do banco</th>
				</tr>

	      {section name=i loop=$list}
	        <tr  bgcolor = "{if $list[i].index % 2 == 0}F7F7F7{else}WHITE{/if}" >
	        	
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idbanco={$list[i].idbanco}">{$list[i].index}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idbanco={$list[i].idbanco}">{$list[i].nome_banco}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idbanco={$list[i].idbanco}">{$list[i].codigo_banco}</a></td>
	        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
		...
		<table width="100%">
    	<form  action="{$smarty.server.PHP_SELF}?ac=editar&idbanco={$info.idbanco}" method="post" name = "for" id = "for">
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Nome do banco:</td>
					<td><input class="long" type="text" name="litnome_banco" id="litnome_banco" maxlength="100" value="{$info.litnome_banco}"/></td>
				</tr>
				
				<tr>
					<td align="right">Nº do banco:</td>
					<td><input class="short" type="text" name="litcodigo_banco" id="litcodigo_banco" maxlength="5" value="{$info.litcodigo_banco}"/></td>
				</tr>
        ...
        <tr><td>&nbsp;</td></tr>
        	...
        	<td align="center" colspan="2">
        		...
        		<input name="Submit" type="submit" class="botao_padrao" value="EXCLUIR" onClick="return(confDelete('for','{$smarty.server.PHP_SELF}?ac=excluir&idbanco={$info.idbanco}','ATENÇÃO! Confirma a exclusão ?'))" >
        	</td>
        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
      ...
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Nome do banco:</td>
					<td><input class="long" type="text" name="nome_banco" id="nome_banco" maxlength="100" value="{$smarty.post.nome_banco}"/></td>
				</tr>
				
				<tr>
					<td align="right">Nº do banco:</td>
					<td><input class="short" type="text" name="codigo_banco" id="codigo_banco" maxlength="5" value="{$smarty.post.codigo_banco}"/></td>
				</tr>
