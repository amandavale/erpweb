...
{if $flags.okay}
  ...
  {if $flags.action == "listar"}
		...
		{if count($list)}
			...
			<table width="95%" align="center">
				...
				<tr>
					...
					<th align='center'>Motivo do bloqueio</th>
				</tr>

	      {section name=i loop=$list}
	        <tr  bgcolor = "{if $list[i].index % 2 == 0}F7F7F7{else}WHITE{/if}" >
	        	
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idmotivo_bloqueio={$list[i].idmotivo_bloqueio}">{$list[i].index}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idmotivo_bloqueio={$list[i].idmotivo_bloqueio}">{$list[i].motivo_bloqueio}</a></td>
	        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
		...
		<table width="100%">
    	<form  action="{$smarty.server.PHP_SELF}?ac=editar&idmotivo_bloqueio={$info.idmotivo_bloqueio}" method="post" name = "for" id = "for">
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Motivo do bloqueio:</td>
					<td><input class="long" type="text" name="litmotivo_bloqueio" id="litmotivo_bloqueio" maxlength="100" value="{$info.litmotivo_bloqueio}"/></td>
				</tr>
        ...
        <tr><td>&nbsp;</td></tr>
        	...
        	<td align="center" colspan="2">
        		...
        		<input name="Submit" type="submit" class="botao_padrao" value="EXCLUIR" onClick="return(confDelete('for','{$smarty.server.PHP_SELF}?ac=excluir&idmotivo_bloqueio={$info.idmotivo_bloqueio}','ATENÇÃO! Confirma a exclusão ?'))" >
        	</td>
        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
      ...
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Motivo do bloqueio:</td>
					<td><input class="long" type="text" name="motivo_bloqueio" id="motivo_bloqueio" maxlength="100" value="{$smarty.post.motivo_bloqueio}"/></td>
				</tr>
