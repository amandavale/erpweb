...
{if $flags.okay}
  ...
  {if $flags.action == "listar"}
		...
		{if count($list)}
	      ...
	      {section name=i loop=$list}
	        <tr  bgcolor = "{if $list[i].index % 2 == 0}F7F7F7{else}WHITE{/if}" >
						...
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idprogramacao_status={$list[i].idprogramacao_status}">{$list[i].index}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idprogramacao_status={$list[i].idprogramacao_status}">{$list[i].nome_programacao}</a></td>
	        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
		...
		<table width="100%">
    	<form  action="{$smarty.server.PHP_SELF}?ac=editar&idprogramacao_status={$info.idprogramacao_status}" method="post" name = "for" id = "for">
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Programação:</td>
					<td><input class="long" type="text" name="litnome_programacao" id="litnome_programacao" maxlength="45" value="{$info.litnome_programacao}"/></td>
				</tr>
        ...
        <tr><td>&nbsp;</td></tr>
        	...
        	<td align="center" colspan="2">
        		...
        		<input name="Submit" type="submit" class="botao_padrao" value="EXCLUIR" onClick="return(confDelete('for','{$smarty.server.PHP_SELF}?ac=excluir&idprogramacao_status={$info.idprogramacao_status}','ATENÇÃO! Confirma a exclusão ?'))" >
        	</td>
        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
      ...
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Programação:</td>
					<td><input class="long" type="text" name="nome_programacao" id="nome_programacao" maxlength="45" value="{$smarty.post.nome_programacao}"/></td>
				</tr>
