...
<table align="center">
			<tr>
				<td colspan="9" align="center">Estado:
					...
					<input class="long" type="text" name="idestado_Nome" id="idestado_Nome" value="{$smarty.post.idestado_Nome}"
						onKeyUp="javascript:
							VerificaMudancaCampo('idestado', 'idcidade#idbairro');
						"
					/>
			...
			<tr>
				<td colspan="9" align="center">Cidade:
					<input type="hidden" name="idcidade" id="idcidade" value="{$smarty.post.idcidade}" />
					<input type="hidden" name="idcidade_NomeTemp" id="idcidade_NomeTemp" value="{$smarty.post.idcidade_NomeTemp}" />
					<input class="long" type="text" name="idcidade_Nome" id="idcidade_Nome" value="{$smarty.post.idcidade_Nome}"
						onKeyUp="javascript:
							VerificaMudancaCampo('idcidade','idbairro');
						"
					/>
			...
			<script type="text/javascript">
			    new CAPXOUS.AutoComplete("idcidade_Nome", function() {ldelim}
			    	return "cidade_ajax.php?ac=busca_cidade&typing=" + this.text.value + "&campoID=idcidade" + "&idestado=" + document.getElementById('idestado').value;
			    {rdelim}, {ldelim} minChars: {$conf.minimo_auto_completar} {rdelim});
			</script>


			<tr>
				<td colspan="9" align="center">Bairro:&nbsp;
					<input type="hidden" name="idbairro" id="idbairro" value="{$smarty.post.idbairro}" />
					<input type="hidden" name="idbairro_NomeTemp" id="idbairro_NomeTemp" value="{$smarty.post.idbairro_NomeTemp}" />
					<input class="long" type="text" name="idbairro_Nome" id="idbairro_Nome" value="{$smarty.post.idbairro_Nome}"
						onKeyUp="javascript:
							VerificaMudancaCampo('idbairro');
						"
					/>
					<span class="nao_selecionou" id="idbairro_Flag">
					&nbsp;&nbsp;&nbsp;
					</span>
			<script type="text/javascript">
			    new CAPXOUS.AutoComplete("idbairro_Nome", function() {ldelim}
			    	return "bairro_ajax.php?ac=busca_bairro&typing=" + this.text.value + "&campoID=idbairro" + "&idcidade=" + document.getElementById('idcidade').value + "&mostraDetalhes=1";
			    {rdelim}, {ldelim} minChars: {$conf.minimo_auto_completar} {rdelim});
			</script>
		...
		{if count($list)}
	      ...
	      {section name=i loop=$list}
	        <tr  bgcolor = "{if $list[i].index % 2 == 0}F7F7F7{else}WHITE{/if}" >
						...
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idbairro={$list[i].idbairro}">{$list[i].index}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idbairro={$list[i].idbairro}">{$list[i].nome_estado}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idbairro={$list[i].idbairro}">{$list[i].nome_cidade}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idbairro={$list[i].idbairro}">{$list[i].nome_bairro}</a></td>
	        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
		...
		<table width="100%">
    	<form  action="{$smarty.server.PHP_SELF}?ac=editar&idbairro={$info.idbairro}" method="post" name = "for" id = "for">
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					...
					<td>
						<input type="hidden" name="idestado" id="idestado" value="{$info.idestado}" />
				...
				<tr>
					<td class="req" align="right">Cidade:</td>
					<td>
						<input type="hidden" name="numidcidade" id="idcidade" value="{$info.numidcidade}" />
						<input type="hidden" name="idcidade_NomeTemp" id="idcidade_NomeTemp" value="{$info.idcidade_NomeTemp}" />
						<input class="long" type="text" name="idcidade_Nome" id="idcidade_Nome" value="{$info.idcidade_Nome}"
							onKeyUp="javascript:
								VerificaMudancaCampo('idcidade');
							"
						/>
						<span class="selecionou" id="idcidade_Flag">
							&nbsp;&nbsp;&nbsp;
						</span>
					</td>
				</tr>
				...
				<tr>
					<td class="req" align="right">Nome do bairro:</td>
					<td><input class="long" type="text" name="litnome_bairro" id="litnome_bairro" maxlength="100" value="{$info.litnome_bairro}"/></td>
				</tr>
        ...
        <tr><td>&nbsp;</td></tr>
        	...
        	<td align="center" colspan="2">
        		...
        		<input name="Submit" type="submit" class="botao_padrao" value="EXCLUIR" onClick="return(confDelete('for','{$smarty.server.PHP_SELF}?ac=excluir&idbairro={$info.idbairro}','ATENÇÃO! Confirma a exclusão ?'))" >
        	</td>
        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
      ...
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Nome do bairro:</td>
					<td><input class="long" type="text" name="nome_bairro" id="nome_bairro" maxlength="100" value="{$smarty.post.nome_bairro}"/></td>
				</tr>
