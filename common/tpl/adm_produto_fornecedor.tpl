...
{if $flags.okay}
	...
	*}
    ...
    {if $flags.sucesso != ""}
      ...
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
        ...
        <tr bgcolor="#F7F7F7">
					<td colspan="9" align="center">Selecione os Fornecedores que fornecem este Produto.</td>
        </tr>
        ...
        <tr>
	      ...
	      {section name=i loop=$list_fornecedor}
	        <tr>
	          <td>
							<input class="radio" type="checkbox" name="fornecedor_{$list_fornecedor[i].idfornecedor}" id="fornecedor_{$list_fornecedor[i].idfornecedor}" {if $list_fornecedor[i].selecionado == 1}checked{/if} />
	          	{$list_fornecedor[i].nome_fornecedor}
						</td>
	        </tr>
	      {/section}
        ...
        <tr><td>&nbsp;</td></tr>
          ...
          <td colspan="2" align="center">
  						<input type='Submit'  class="botao_padrao" value="ATUALIZAR" name = "ATUALIZAR" id = "ATUALIZAR" />
          </td>
        </tr>
  ...
  {if $flags.action == "listar"}
		...
		{if count($list)}
	      ...
	      {section name=i loop=$list}
	        <tr  bgcolor = "{if $list[i].index % 2 == 0}F7F7F7{else}WHITE{/if}" >
						...
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idproduto={$list[i].idproduto}&idfornecedor={$list[i].idfornecedor}">{$list[i].index}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idproduto={$list[i].idproduto}&idfornecedor={$list[i].idfornecedor}">{$list[i].descricao_produto}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idproduto={$list[i].idproduto}&idfornecedor={$list[i].idfornecedor}">{$list[i].nome_fornecedor}</a></td>
	        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
		...
		<table width="100%">
    	<form  action="{$smarty.server.PHP_SELF}?ac=editar&idproduto={$info.idproduto}&idfornecedor={$info.idfornecedor}" method="post" name = "for" id = "for">
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
        ...
        <tr><td>&nbsp;</td></tr>
        	...
        	<td align="center" colspan="2">
        		...
        		<input name="Submit" type="submit" class="botao_padrao" value="EXCLUIR" onClick="return(confDelete('for','{$smarty.server.PHP_SELF}?ac=excluir&idproduto={$info.idproduto}&idfornecedor={$info.idfornecedor}','ATENÇÃO! Confirma a exclusão ?'))" >
        	</td>
        </tr>
