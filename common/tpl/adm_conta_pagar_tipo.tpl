...
{if $flags.okay}
  ...
  {if $flags.action == "listar"}
		...
		{if count($list)}
			...
			<table width="95%" align="center">
				...
				<tr>
					...
					<th align='center'>Descrição da conta a pagar</th>
				</tr>

	      {section name=i loop=$list}
	        <tr  bgcolor = "{if $list[i].index % 2 == 0}F7F7F7{else}WHITE{/if}" >
	        	
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idconta_pagar_tipo={$list[i].idconta_pagar_tipo}">{$list[i].index}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idconta_pagar_tipo={$list[i].idconta_pagar_tipo}">{$list[i].descricao_conta_pagar}</a></td>
	        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
		...
		<table width="100%">
    	<form  action="{$smarty.server.PHP_SELF}?ac=editar&idconta_pagar_tipo={$info.idconta_pagar_tipo}" method="post" name = "for" id = "for">
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Descrição da conta a pagar:</td>
					<td><input class="long" type="text" name="litdescricao_conta_pagar" id="litdescricao_conta_pagar" maxlength="50" value="{$info.litdescricao_conta_pagar}"/></td>
				</tr>
        ...
        <tr><td>&nbsp;</td></tr>
        	...
        	<td align="center" colspan="2">
        		...
        		<input name="Submit" type="submit" class="botao_padrao" value="EXCLUIR" onClick="return(confDelete('for','{$smarty.server.PHP_SELF}?ac=excluir&idconta_pagar_tipo={$info.idconta_pagar_tipo}','ATENÇÃO! Confirma a exclusão ?'))" >
        	</td>
        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
      ...
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Descrição da conta a pagar:</td>
					<td><input class="long" type="text" name="descricao_conta_pagar" id="descricao_conta_pagar" maxlength="50" value="{$smarty.post.descricao_conta_pagar}"/></td>
				</tr>
