...
{if $flags.okay}
  ...
  {if $flags.action == "listar"}
		...
		{if count($list)}
			...
			<table width="95%" align="center">
				...
				<tr>
					...
					<th align='center'>Tipo de Serviço</th>
				</tr>

	      {section name=i loop=$list}
	        <tr  bgcolor = "{if $list[i].index % 2 == 0}F7F7F7{else}WHITE{/if}" >
	        	
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idtipo_servico={$list[i].idtipo_servico}">{$list[i].index}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idtipo_servico={$list[i].idtipo_servico}">{$list[i].nome_servico}</a></td>
	        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
		...
		<table width="100%">
    	<form  action="{$smarty.server.PHP_SELF}?ac=editar&idtipo_servico={$info.idtipo_servico}" method="post" name = "for" id = "for">
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Tipo de Serviço:</td>
					<td><input class="long" type="text" name="litnome_servico" id="litnome_servico" maxlength="100" value="{$info.litnome_servico}"/></td>
				</tr>
        ...
        <tr><td>&nbsp;</td></tr>
        	...
        	<td align="center" colspan="2">
        		...
        		<input name="Submit" type="submit" class="botao_padrao" value="EXCLUIR" onClick="return(confDelete('for','{$smarty.server.PHP_SELF}?ac=excluir&idtipo_servico={$info.idtipo_servico}','ATENÇÃO! Confirma a exclusão ?'))" >
        	</td>
        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
      ...
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Tipo de Serviço:</td>
					<td><input class="long" type="text" name="nome_servico" id="nome_servico" maxlength="100" value="{$smarty.post.nome_servico}"/></td>
				</tr>
