    ...
    <input type="hidden" name="for_chk" id="for_chk" value="1" />
		...
		{if count($list_movimentacao)} 
			...
			<table border="0" width="100%">
				...
				<tr>
					<th align='center'>Código do Orçamento</th>
          <th align='center'>Data do Orçamento</th>   					
          <th align='center'>Funcionário Responsável</th>
          <th align='center'>Cliente</th>
					<th align='center'>Valor do Orçamento (R$)</th>
				</tr>
				...
				{section name=i loop=$list_movimentacao}
					...
					<tr>
						<td align='center'><a class='menu_item' href = "{$conf.addr}/admin/orcamento.php?ac=editar&idorcamento={$list_movimentacao[i].idorcamento}">{$list_movimentacao[i].idorcamento}</a></td>
            <td align='center'><a class='menu_item' href = "{$conf.addr}/admin/orcamento.php?ac=editar&idorcamento={$list_movimentacao[i].idorcamento}">{$list_movimentacao[i].datahoraCriacao}</a></td>						
            <td align='left'><a class='menu_item' href = "{$conf.addr}/admin/orcamento.php?ac=editar&idorcamento={$list_movimentacao[i].idorcamento}">{$list_movimentacao[i].nome_funcionario}</a></td>
						<td align='left'><a class='menu_item' href = "{$conf.addr}/admin/orcamento.php?ac=editar&idorcamento={$list_movimentacao[i].idorcamento}">{$list_movimentacao[i].nome_cliente}</a></td>
            <td align='right'><a class='menu_item' href = "{$conf.addr}/admin/orcamento.php?ac=editar&idorcamento={$list_movimentacao[i].idorcamento}">{$list_movimentacao[i].valor_total_nota}</a></td>	
					</tr>
