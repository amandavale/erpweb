...
{if $flags.okay}
  ...
  {if $flags.action == "listar"}
		...
		{if count($list)}
			...
			<table width="95%" align="center">
				...
				<tr>
					...
					<th align='center'>Nome do cargo</th>
				</tr>

	      {section name=i loop=$list}
	        <tr  bgcolor = "{if $list[i].index % 2 == 0}F7F7F7{else}WHITE{/if}" >
	        	
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idcargo={$list[i].idcargo}">{$list[i].index}</a></td>
						<td><a class='menu_item' href = "{$smarty.server.PHP_SELF}?ac=editar&idcargo={$list[i].idcargo}">{$list[i].nome_cargo}</a></td>
	        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
		...
		<table width="100%">
    	<form  action="{$smarty.server.PHP_SELF}?ac=editar&idcargo={$info.idcargo}" method="post" name = "for" id = "for">
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Nome do cargo:</td>
					<td><input class="long" type="text" name="litnome_cargo" id="litnome_cargo" maxlength="100" value="{$info.litnome_cargo}"/></td>
				</tr>
				
				<tr>
					<td align="right">Observação:</td>
					<td>
						<textarea name="litobservacao_cargo" id="litobservacao_cargo" rows='6' cols='38'>{$info.litobservacao_cargo}</textarea>
					</td>
				</tr>
        ...
        <tr><td>&nbsp;</td></tr>
        	...
        	<td align="center" colspan="2">
        		...
        		<input name="Submit" type="submit" class="botao_padrao" value="EXCLUIR" onClick="return(confDelete('for','{$smarty.server.PHP_SELF}?ac=excluir&idcargo={$info.idcargo}','ATENÇÃO! Confirma a exclusão ?'))" >
        	</td>
        </tr>
    ...
    {include file="div_instrucoes_fim.tpl"}
      ...
      <input type="hidden" name="for_chk" id="for_chk" value="1" />
				...
				<tr>
					<td class="req" align="right">Nome do cargo:</td>
					<td><input class="long" type="text" name="nome_cargo" id="nome_cargo" maxlength="100" value="{$smarty.post.nome_cargo}"/></td>
				</tr>
				
				<tr>
					<td align="right">Observação:</td>
					<td>
						<textarea name="observacao_cargo" id="observacao_cargo" rows='6' cols='38'>{$smarty.post.observacao_cargo}</textarea>
					</td>
				</tr>
